# coding: utf-8

"""
    FastSpring API

    The FastSpring API and its supported requests, data, endpoints, and rate limits.  The FastSpring API is a backend service you can use to communicate with your FastSpring Store programmatically. It requires basic understanding of HTTP communication methods. The primary API communication methods include: - **GET** - request data from your store - **POST** - send data to your store, such as creating a new product record - **DELETE** - remove data from your store  The structure of the JSON data sent and received using the API is very similar to Webhooks. However, using the FastSpring API, you initiate all communication with your FastSpring Store.  If you prefer a client-side method of communicating with your FastSpring Store and generating dynamic webpage content, check out the [Store Builder Library](https://fastspring.com/docs/get-started-with-store-builder-library/).  ## Access the API  #### API Credentials and Authentication  FastSpring API credentials consist of a single username and password for your entire Store. The username and password are used for [Basic Authentication](https://en.wikipedia.org/wiki/Basic_access_authentication) when making requests to the API.  #### Obtain Your API Credentials  1. Log on to the FastSpring App and navigate to [Integrations](https://fastspring.com/docs/integrations) > **API Credentials**. Initially, the only option on this page is the **Create** command.  2. Click **Create** to generate your API **Username** and **Password**. The page automatically refreshes, and then your credentials are displayed.  3. Make a note of the credentials and store them securely.  The API credentials allow complete access and control over your FastSpring Store. You will not be able to view the password in the FastSpring App after this session. If you forget your API credentials, you can reset them, however any of your stored procedures that rely on API access must be updated immediately with the new credentials. Otherwise, the API requests will fail.  ## API Requests  **Make all requests to https://api.fastspring.com**  When you make your requests, consider the following: - Ensure that the HTTP methods for all calls use uppercase letters (“GET,” “POST,” “DELETE”) Lowercase letters may result in a 404 error response. - FastSpring requires TLS 1.2 (or later) encryption for all calls to the FastSpring API.  - The API requires a **User-Agent** header in all requests. If your request does not include it, you may receive an 401 error message when attempting to make API requests. - The API uses basic authentication and does not support URL-encoded authentication. Use **Base64** to encode your username and password in the header. 

    The version of the OpenAPI document: 1.0
    Generated by: https://konfigthis.com
"""

from datetime import datetime, date
import typing
from enum import Enum
from typing_extensions import TypedDict, Literal, TYPE_CHECKING
from pydantic import BaseModel, Field, RootModel

from fast_spring_python_sdk.pydantic.address_response import AddressResponse
from fast_spring_python_sdk.pydantic.contact_response import ContactResponse
from fast_spring_python_sdk.pydantic.item_response import ItemResponse
from fast_spring_python_sdk.pydantic.link import Link
from fast_spring_python_sdk.pydantic.status_history_response import StatusHistoryResponse
from fast_spring_python_sdk.pydantic.tag_response import TagResponse

class QuoteResponse(BaseModel):
    tags: typing.Optional[typing.List[TagResponse]] = Field(None, alias='tags')

    # The quote id
    id: typing.Optional[str] = Field(None, alias='id')

    buyer_generated: typing.Optional[bool] = Field(None, alias='buyerGenerated')

    coupon: typing.Optional[str] = Field(None, alias='coupon')

    created: typing.Optional[datetime] = Field(None, alias='created')

    created_by: typing.Optional[ContactResponse] = Field(None, alias='createdBy')

    currency: typing.Optional[str] = Field(None, alias='currency')

    discount: typing.Optional[typing.Union[int, float]] = Field(None, alias='discount')

    discount_display: typing.Optional[str] = Field(None, alias='discountDisplay')

    discount_in_payout_currency: typing.Optional[typing.Union[int, float]] = Field(None, alias='discountInPayoutCurrency')

    discount_in_payout_currency_display: typing.Optional[str] = Field(None, alias='discountInPayoutCurrencyDisplay')

    expires: typing.Optional[datetime] = Field(None, alias='expires')

    expiration_date_days: typing.Optional[int] = Field(None, alias='expirationDateDays')

    fulfillment_term: typing.Optional[Literal["ON_PAYMENT", "ON_QUOTE_ACCEPTANCE"]] = Field(None, alias='fulfillmentTerm')

    items: typing.Optional[typing.List[ItemResponse]] = Field(None, alias='items')

    name: typing.Optional[str] = Field(None, alias='name')

    notes: typing.Optional[str] = Field(None, alias='notes')

    net_terms_days: typing.Optional[int] = Field(None, alias='netTermsDays')

    quote_url: typing.Optional[str] = Field(None, alias='quoteUrl')

    recipient: typing.Optional[ContactResponse] = Field(None, alias='recipient')

    recipient_address: typing.Optional[AddressResponse] = Field(None, alias='recipientAddress')

    site_id: typing.Optional[str] = Field(None, alias='siteId')

    status: typing.Optional[Literal["OPEN", "CANCELED", "AWAITING_PAYMENT", "COMPLETED", "EXPIRED"]] = Field(None, alias='status')

    status_history: typing.Optional[typing.List[StatusHistoryResponse]] = Field(None, alias='statusHistory')

    subtotal: typing.Optional[typing.Union[int, float]] = Field(None, alias='subtotal')

    subtotal_display: typing.Optional[str] = Field(None, alias='subtotalDisplay')

    subtotal_in_payout_currency: typing.Optional[typing.Union[int, float]] = Field(None, alias='subtotalInPayoutCurrency')

    subtotal_in_payout_currency_display: typing.Optional[str] = Field(None, alias='subtotalInPayoutCurrencyDisplay')

    tax: typing.Optional[typing.Union[int, float]] = Field(None, alias='tax')

    tax_type: typing.Optional[str] = Field(None, alias='taxType')

    total: typing.Optional[typing.Union[int, float]] = Field(None, alias='total')

    total_display: typing.Optional[str] = Field(None, alias='totalDisplay')

    total_in_payout_currency: typing.Optional[typing.Union[int, float]] = Field(None, alias='totalInPayoutCurrency')

    total_in_payout_currency_display: typing.Optional[str] = Field(None, alias='totalInPayoutCurrencyDisplay')

    updated: typing.Optional[datetime] = Field(None, alias='updated')

    tax_id: typing.Optional[str] = Field(None, alias='taxId')

    source: typing.Optional[str] = Field(None, alias='source')

    source_i_p: typing.Optional[str] = Field(None, alias='sourceIP')

    is_gross_tax: typing.Optional[bool] = Field(None, alias='isGrossTax')

    invoice_id: typing.Optional[str] = Field(None, alias='invoiceId')

    links: typing.Optional[typing.List[Link]] = Field(None, alias='links')
    class Config:
        arbitrary_types_allowed = True
