...
from fast_spring_python_sdk.pydantic.discount_reason import DiscountReason
from fast_spring_python_sdk.pydantic.price import Price
from fast_spring_python_sdk.pydantic.pricing1_quantity_discounts import Pricing1QuantityDiscounts

class Pricing1(BaseModel):
    trial: int = Field(alias='trial')

    interval: str = Field(alias='interval')

    interval_length: int = Field(alias='intervalLength')

    quantity_behavior: str = Field(alias='quantityBehavior')

    quantity_default: int = Field(alias='quantityDefault')

    price: Price = Field(alias='price')

    quantity_discounts: Pricing1QuantityDiscounts = Field(alias='quantityDiscounts')

    discount_reason: DiscountReason = Field(alias='discountReason')

    discount_duration: int = Field(alias='discountDuration')

    payment_collected: typing.Optional[bool] = Field(None, alias='paymentCollected')

    paid_trial: typing.Optional[bool] = Field(None, alias='paidTrial')

    trial_price: typing.Optional[Price] = Field(None, alias='trialPrice')
    class Config:
